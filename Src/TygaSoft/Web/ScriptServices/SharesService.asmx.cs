...
namespace TygaSoft.Web.ScriptServices
{
    ...
    public class SharesService : System.Web.Services.WebService
    {
        ...
        public string Login(string userName, string psw,string vc)
        {
            ...
            if (!Regex.IsMatch(psw, @"(([0-9]+)|([a-zA-Z]+)){6,30}"))
            {
                return MessageContent.Request_InvalidPassword;
            }

            if (vc.ToLower() != context.Request.Cookies["ValidateCode"]["DlgLoginVc"].ToLower())
            {
                return MessageContent.GetString(MessageContent.Request_InvalidValue, "验证码");
            }
